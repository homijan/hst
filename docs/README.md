...
$`\begin{align}
S_\pi[f](\tau) &= \int_{\tau_0}^{\tau} L[f, \dot{f}] d\tau = \int_{\tau_0}^{\tau} \int_\Omega \mathcal{L}(f(a, \tau), \dot{f}(a, \tau), a, \tau) d^3a~d\tau 
\\
&\overset{^{ideal}_{fluid}}{=} \int_{\tau_0}^{\tau} \int_\Omega \rho_0(a) \left( \frac{1}{2}\dot{f}(a, \tau)^2 - \varepsilon(a, \tau) \right) d^3 a~d\tau \overset{_{\pi = \rho_0 \dot{f}}}{=} \int_{\tau_0}^{\tau} \int_\Omega \left( \frac{1}{2 \rho_0(a)} \pi(a, \tau)^2 - \rho_0(a) \varepsilon(a, \tau) \right) d^3 a~d\tau,~(l1)
\end{align}`$
